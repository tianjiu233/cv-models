...
# my libs
from dataio import OrchardDataset
...
def show_certain_picture(idx,dataset,trainer):
    sample = dataset[idx]
...
if __name__ == "__main__":
    ...
    train_dir = "/home/huijian/exps/Data/orchard_sample/train"
    test_dir = "/home/huijian/exps/Data/orchard_sample/test"
    
    composed = transforms.Compose([RandomCrop(256)])
    
    file_path = "/home/huijian/exps/segmentation_unet/model/"
    train_data = OrchardDataset(root_dir = train_dir, transform = composed)
    test_data = OrchardDataset(root_dir = test_dir, transform = composed)
    ...
    test_data = OrchardDataset(root_dir=test_dir, transform = transforms.Compose([RandomCrop(1024)]))
    # show result:
    if True:
        show_certain_picture(idx=6, dataset=test_data, trainer=trainer)
        """
        sample=test_data[4]
        image_pred = trainer.predict(Variable(torch.FloatTensor(sample["image"])).unsqueeze(0))
        image_pred = image_pred.squeeze().data.numpy()
