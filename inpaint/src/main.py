...
if __name__=="__main__":
    ...
    # some parameters for erase process and training process
    ...
    # 2. networks, batch_Size and optimizers
    in_chs =3
    out_chs =3
    drop_ratio =0.75
    ...
    coach_optimizer = optim.SGD(net.parameters(),lr=0.1, momentum=0.9, weight_decay=5e-4)
    ...
    inpaint_train_dataloader = data.DataLoader(dataset = inpaint_train_data,
                                               batch_size=inpaint_train_bs,shuffle=True)
    ...
    epochs = [100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30]
    ...
    for e_ in range(0,len(epochs)):
        ...
        for epoch in range(epochs[e_]):
            ...
            _visualize_inpaint(inpaint_val_dataloader,
                       ...
                       prefix = "epoch_"+str(e_+1) + "_"+str(epoch+1)+"_",
                       cuda=cuda,device=device,
                       pic_dir = "../temp")
    ...
    for e in range(epochs):
        ...
        if (e+1)%10 == 0:
            ...
            with open("train-info.txt","a") as file_handle:    
                ...
                file_handle.write("\n")
            _save_model(coach,model_name="coach"+"_epoch_"+str(e+1),model_path = "../checkpoint",cuda=cuda,device=device)
            _save_model(net,model_name="net"+"_epoch_"+str(e+1),model_path = "../checkpoint",cuda=cuda,device=device)
            # save image
            _visualize_inpaint(inpaint_val_dataloader,
                       net=net,coach=coach,
                       stats = AerialImageDataset_stats,
